...
export const SparkleButton: React.FC<SparkleButtonProps> = ({ 
  ...
  const particlePenRef = useRef<HTMLSpanElement>(null);

  useEffect(() => {
    if (particlePenRef.current) {
  ...
  const ParticlePen = () => (
    <span aria-hidden="true" className="particle-pen" ref={particlePenRef}>
  ...
  if (href) {
    return (
      <div className={`sparkle-button ${className}`}>
        <a href={href} className="sparkle-button-link">
